 ...
 * Modelo Convenios
 */
class Convenios extends Models implements IModels
{
    ...
    public function setControl()
    {
        ...
        # SETEAR VALOR ID CONVENIO

        $u = new Model\Users;
        $this->id_convenio = $u->getOwnerUser()['id_convenio'];
        $this->name_convenio = $u->getOwnerUser()['name_convenio'];
    ...
    public function getResultadosLab()
    {
        ...
        try {
            ...
            $this->name_convenio = (int) $http->query->get('nameConvenio');

            $this->id_convenio = (int) $http->query->get('idConvenio');
            ...
            if ($_searchField != false && $this->isRange($http->query->get('search')['value']) === false) {
                ...
                AND COD_CONVENIO = '" . $this->id_convenio . "'
                AND TOT_SC != TOD_DC
                AND ( CEDULA LIKE '" . $this->searchField . "%' OR NOMBRE_PERSONA LIKE '%" . $this->searchField . "%' )
                ORDER BY FECHA DESC
                )
    ...
    public function getResultadosLabById()
    {
        ...
        try {
            ...
            $id_resultado = $http->query->get('idDocumento');

            $fecha = $http->query->get('fechaDocumento');
...
// Private funtion edit pdf
    public function loadsPDF()
    {

        $archivos = Helper\Files::get_files_in_dir('../api/documentos/', 'pdf');

        foreach ($archivos as $key => $value) {

            $archivo = explode('/', $value);
            $pdf = explode('.', $archivo[3]);

            if ($key <= 100) {
                $proceso = $this->editPDF($pdf[0]);
                if ($proceso) {
    ...
    public function editPDF($archivo)
    {

        try {

            # setear imagen con texto para footer
            $alexandra = "../api/documentos/alexandra.png";

            # setear imagen con texto para footer
            $alfonso = "../api/documentos/alfonso.png";

            $pdf = new Fpdi();

            $pdf->AddPage();
            $pdf->setSourceFile('../api/documentos/' . $archivo . '.pdf');
            $template = $pdf->importPage(1);
            $pdf->useTemplate($template);

            # Hm
            $pdf->Image($alexandra, 27, 243, 5, 15);
            $pdf->Image($alfonso, 140, 242, 16, 12.5);

            # Metrored
            # $pdf->Image($alfonso, 22, 243, 24, 15);
            # $pdf->Image($alexandra, 140, 243, 18, 11);

            $pdf->Output("F", '../api/firmados/' . $archivo . '.firmado.pdf');

            # Eliminar archivo
            Helper\Files::delete_file('../api/documentos/' . $archivo . '.pdf');
