...
class Orders extends Models implements IModels
{
    ...
    public function registroPedidoElectronicoWeb_LAB()
    {
        ...
        try {
            ...
            $this->pedido_fk_medico = $this->USER->CP_MED[0];
    ...
    public function getPedidos_Medicos_Paciente()
    {
        ...
        try {
            ...
            foreach ($data as $key) {
                ...
                $key['PRECIO']       = $this->getValorTotalPedido($key['PK_CODIGO']);
                $key['FECHA']        = date('d-m-Y', strtotime($key['FECHA']));
                $key['DETALLE']      = $this->getDetallePedido($key['PK_CODIGO']);
    ...
    public function getPedidos_Medicos()
    {
        ...
        try {
            ...
            ccp_cab_pedidos_medico WHERE FK_MEDICO IN ($codes) ORDER BY $this->sortField $this->sortType ";
    ...
    private function getDetallePedido($id_pedido)
    {
        # CONULTA BDD GEMA
        $sql = "SELECT PVP, COD_ARTIC FROM
    ...
    private function getValorTotalPedido($id_pedido)
    {
        # CONULTA BDD GEMA
        $sql = "SELECT PVP FROM
            ccp_det_pedidos_medico WHERE PK_FK_PEDIDO='$id_pedido'";
    ...
    private function getEstado_Abono($id_trx)
    {
        ...
        $sql = "SELECT a.pk_numero_transaccion_credito as trx
                ...
                and t.identificacion_titular='$this->USER' and a.pk_numero_transaccion_credito='$id_trx'";
