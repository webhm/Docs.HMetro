    ...
    public function obtenerResultadosMedicoHM()
    {
        ...
        try {
            ...
            $response = $client->GetResults(array(
                ...
                "pstrOrderDateFrom" => date('Y-m-d'),
                "pstrOrderDateTo" => date('Y-m-d'),
                "pintMinStatusTest" => "5",
                "pintMaxStatusTest" => "5",
            ));
            ...
            foreach ($response->GetResultsResult->Orders->LISOrder as $key => $val) {
                if ($val->Doctor == $this->id_user->codMedico) {

                    $id_resultado = Helper\Strings::ocrend_encode($val->SampleID, $this->hash);

                    $resultados[] = array(
                        'ID_RESULTADO' => $id_resultado,
                        'CC' => $val->OptionalDemogList->LISElementValue[11]->Value,
                        'NOMBRE_PERSONA' => "PACIENTE CC: " . $val->OptionalDemogList->LISElementValue[11]->Value,
                        'FECHA' => $val->RegisterDate,
    ...
    public function obtenerResultadoLabHM()
    {
        ...
        try {
            ...
            return array(
                ...
                'id_resultado' => $id_resultado,
                'pdf' => $config['build']['url'] . 'v1/documentos/resultados/' . $idResultado . ".pdf",
            );
